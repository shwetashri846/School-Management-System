...
def updatetgridview():
    if request.method == 'POST':
        ...
        cur.execute("""
               ...
               SET  name=%s,class=%s,stream=%s,add_section=%s
               WHERE id=%s
            """, (  name,add_class,stream, add_section,id_data))
        mysql.connection.commit()
        msg="successfully updated"
        return render_template("Dashboard.html" ,msg=msg)
...
def teacherreport ():
        cur=mysql.connection.cursor()
        cur.execute('SELECT id, qualification FROM addteacher')
        data=cur.fetchall()
...
def studentrep ():
        cur=mysql.connection.cursor()
        cur.execute('SELECT id, name,class,stream,add_section FROM studentreport')
        data=cur.fetchall()
        return render_template('studentgridview.html',studentreport=data)
...
def nonfaculty():
    ...
    if request.method == 'POST': 
        ...
        password = request.form['password']
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT * FROM nonfaclog WHERE username = % s AND password = % s', (username, password, ))
        ...
        if nonfaclog:
            ...
            msg = 'Wrong Credential !'
            return render_template('nonFacultyLogin.html', msg = msg)
    else:
        return render_template("nonFacultyLogin.html")
...
def teacherlogin():
    ...
    if request.method == 'POST': 
        ...
        password = request.form['password']
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT * FROM teacherlogin WHERE username = % s AND password = % s', (username, password, ))
...
def deletestudentregistrationreport(id_data):
    ...
    cur = mysql.connection.cursor()
    cur.execute("DELETE FROM addstudentdetail WHERE id=%s", (id_data,))
    mysql.connection.commit()
    msg=("Record Has Been Deleted Successfully")
    return render_template("Dashboard.html" , msg=msg)
...
def time():
    if request.method=="POST":
        ...
        cur=mysql.connection.cursor()
        cur.execute("INSERT INTO time (time,time1)VALUES(%s,%s)" ,[time,time])
        mysql.connection.commit()
...
def addsubject():
    if request.method=="POST":
        ...
        i=cur.fetchall()
        return render_template('addsubject.html',add_class=h,addsubject=i)



@app.route('/classlecture',methods=['GET','POST'])
def classlecture():
        if request.method=="POST":
            ...
            cur=mysql.connection.cursor()
            cur.execute("INSERT INTO classlecture (topic,subject,name,description)VALUES(%s,%s,%s,%s)" ,[topic,subject,name,description])
            mysql.connection.commit()
            ...
            k=cur.fetchall()
            return render_template('classlecture.html',addsyllabus=i,addsubject=h,classlecture=j,addteacher=k)
...
def routine():
    if request.method=="POST":
        ...
        cur=mysql.connection.cursor()
        cur.execute("INSERT INTO managesection (class,addsection ,addsubject,periodname,time,time1)VALUES(%s,%s,%s,%s,%s,%s,%s)" ,[classname,classsection,addsubject,periodname,teachername,time,time1])
        mysql.connection.commit()
        msg="Successfully routine updated"
        return render_template("Dashboard.html" ,msg=msg)
    else:
        cur=mysql.connection.cursor()
        cur.execute("SELECT class FROM add_class ORDER BY id")
        j=cur.fetchall()
        cur.execute("SELECT subject FROM addsubject ORDER BY id")
        k=cur.fetchall()
        cur.execute("SELECT add_section FROM new_section ORDER BY id")
        j=cur.fetchall()
        cur.execute("SELECT name FROM addteacher ORDER BY id")
        l=cur.fetchall()
        cur.execute("SELECT time,time1 FROM time ORDER BY id")
        m=cur.fetchall()
        cur.execute("SELECT time1 FROM time ORDER BY id")
        n=cur.fetchall()
        return render_template('routine.html',add_class=j,addsubject=k,addteacher=l,time=m,time1=n,new_section=j)
